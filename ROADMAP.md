...
- [ ] refactor `clean_colnames()` to wrap `janitor::make_clean_names()` (more robust than the hand-rolled gsub chain) while keeping the `colname_map` before/after attribute; leave `check_colnames()` custom (report-only — no janitor equivalent). Home skill: `data-qc`. Weigh against the base-R low-dependency lean.
